### Project 21 - The product category has pagination (GET method)
![App Screenshot](./project21/assets/screenshots/img.png)
### Project 22 - Update user profile with the Upload Avatar feature (POST method)
##### User profile page
![App Screenshot](./project22/assets/screenshots/i1.png)
##### After update user profile
![App Screenshot](./project22/assets/screenshots/i2.png)

### Project 23 - Simple login and user profile with array, Session and Cookies
##### Login page
![App Screenshot](./project23/assets/screenshots/login.png)
##### Error when user not logged in
![App Screenshot](./project23/assets/screenshots/error.png)
##### User profile page
![App Screenshot](./project23/assets/screenshots/profile.png)

### Project 24 - Basic user authentication and authorization with array, Session, Cookies
##### Admin profile page
![App Screenshot](./project24/assets/screenshots/admin-profile.png)
##### User profile page
![App Screenshot](./project24/assets/screenshots/user-profile.png)
##### Update user profile page
![App Screenshot](./project24/assets/screenshots/update-profile.png)
##### Admin panel page
![App Screenshot](./project24/assets/screenshots/admin-panel.png)
##### Edit user page (Admin)
![App Screenshot](./project24/assets/screenshots/update-user.png)

### Project 25 - Building a book database using MySQL and reviewing SQL queries
##### Books table
![App Screenshot](./project25/screenshots/books.png)
##### Authors table
![App Screenshot](./project25/screenshots/authors.png)
##### Books_Authors table
![App Screenshot](./project25/screenshots/books_authors.png)
##### Reviews table
![App Screenshot](./project25/screenshots/reviews.png)
